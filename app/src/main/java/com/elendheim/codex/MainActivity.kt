...
// The single Activity that hosts the whole Compose app. It shows the Elendheim
...
@Composable
private fun Root() {
    // Build the shared ViewModel with the repository from the Application container.
    ...
    if (showSplash) {
        ElendheimSplash(onFinished = { showSplash = false })
    } else {
        CodexNavHost(vm = vm)
    }
}
